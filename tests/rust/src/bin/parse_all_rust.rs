...
use test_rust_roundtrip::{parse_rust, SourceFile};
use treesitter_types::FromNode;

fn main() {
    let dir = std::env::args()
        .nth(1)
        .expect("usage: parse_all_rust <directory>");
    ...
    let mut typed_failed_clean = 0usize; // tree-sitter OK but typed conversion failed
    let mut typed_failed_dirty = 0usize; // tree-sitter had errors AND typed conversion failed

    let start = Instant::now();

    for entry in walk_files(&dir, "rs") {
        total += 1;
        ...
        match SourceFile::from_node(root, &src) {
            Ok(_) => {
                success += 1;
            }
            Err(e) => {
