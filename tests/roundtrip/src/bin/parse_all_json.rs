...
use test_roundtrip::json::Document;
use test_roundtrip::parse_json;
use treesitter_types::FromNode;

fn main() {
    let dir = std::env::args()
        .nth(1)
        .expect("usage: parse_all_json <directory>");
    ...
    for entry in walk_files(&dir, "json") {
        total += 1;
        ...
        match Document::from_node(root, &src) {
            Ok(_) => {
                success += 1;
            }
            Err(e) => {
