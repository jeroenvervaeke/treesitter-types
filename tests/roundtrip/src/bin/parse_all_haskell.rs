...
use test_roundtrip::haskell::Haskell;
use test_roundtrip::parse_haskell;
use treesitter_types::FromNode;

fn main() {
    let dir = std::env::args()
        .nth(1)
        .expect("usage: parse_all_haskell <directory>");
    ...
    for entry in walk_files(&dir, "hs") {
        total += 1;
        ...
        match Haskell::from_node(root, &src) {
            Ok(_) => {
                success += 1;
            }
            Err(e) => {
