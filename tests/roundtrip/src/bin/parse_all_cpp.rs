...
use test_roundtrip::cpp::TranslationUnit;
use test_roundtrip::parse_cpp;
use treesitter_types::FromNode;

fn main() {
    let dir = std::env::args()
        .nth(1)
        .expect("usage: parse_all_cpp <directory>");
    ...
    for entry in walk_files(&dir, "cpp") {
        total += 1;
        ...
        match TranslationUnit::from_node(root, &src) {
            Ok(_) => {
                success += 1;
            }
            Err(e) => {
