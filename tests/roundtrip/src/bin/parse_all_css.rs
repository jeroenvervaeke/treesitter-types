...
use test_roundtrip::css::Stylesheet;
use test_roundtrip::parse_css;
use treesitter_types::FromNode;

fn main() {
    let dir = std::env::args()
        .nth(1)
        .expect("usage: parse_all_css <directory>");
    ...
    for entry in walk_files(&dir, "css") {
        total += 1;
        ...
        match Stylesheet::from_node(root, &src) {
            Ok(_) => {
                success += 1;
            }
            Err(e) => {
