...
use test_roundtrip::c_sharp::CompilationUnit;
use test_roundtrip::parse_c_sharp;
use treesitter_types::FromNode;

fn main() {
    let dir = std::env::args()
        .nth(1)
        .expect("usage: parse_all_c_sharp <directory>");
    ...
    for entry in walk_files(&dir, "cs") {
        total += 1;
        ...
        match CompilationUnit::from_node(root, &src) {
            Ok(_) => {
                success += 1;
            }
            Err(e) => {
